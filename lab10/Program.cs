...
namespace task_7;
    ...
    public class Task7
    {
        ...
        private static Node<T> InnerBuild<T>(T[] arr, int start, int end) where T : IComparable<T>
        {
            ...
            {
                Value = arr[middle],
                Left = InnerBuild(arr, start, middle - 1),
                Right = InnerBuild(arr, middle + 1, end)
            };
            return node;
        }
        /**
        ...
        private static bool IsValidBST<T>(Node<T> node, T minValue, T maxValue) where T : IComparable<T>
        {
            if (node == null)
            {
                return true;
